...
class PaymentRepository implements PaymentRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $students_accounts = new StudentAccounts();
            ...
            DB::commit();
            $notification = array(
                'message' => 'Payment Student Saved successfully',
                'alert-type'=> 'success',
            );
    ...
    public function update($request)
    {
        ...
        try {
            ...
            // حفظ البيانات في جدول حساب الطلاب
            $students_accounts = StudentAccounts::where('payment_id',$request->id)->first();
            $students_accounts->date = date('Y-m-d');
            $students_accounts->type = 'payment';
            $students_accounts->student_id = $request->student_id;
            $students_accounts->payment_id = $payment_students->id;
            ...
            $notification = array(
                'message' => 'Payment Student Updated successfully',
                'alert-type'=> 'success',
            );
            return redirect()->route('Payment_students.index')->with($notification);
        } catch (\Exception $e) {
    ...
    public function destroy($request)
    {
        try {
            ...
            $notification = array(
                'message' => 'Payment Student Deleted successfully',
                'alert-type'=> 'error',
            );
            return redirect()->route('Payment_students.index')->with($notification);
        }
