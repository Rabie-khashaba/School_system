...
class ReceiptStudentsRepository implements ReceiptStudentsRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $fund_accounts = new StudentAccounts();
            $fund_accounts->date = date('Y-m-d');
            ...
            $notification = array(
                'message' => 'Receipt Student Saved successfully',
                'alert-type'=> 'success',
            );
    ...
    public function update($request)
    {
        ...
        try {
            ...
            $fund_accounts = StudentAccounts::where('receipt_id',$request->id)->first();
            $fund_accounts->date = date('Y-m-d');
            $fund_accounts->type = 'receipt';
            $fund_accounts->student_id = $request->student_id;
            $fund_accounts->receipt_id = $receipt_students->id;
            ...
            DB::commit();
            $notification = array(
                'message' => 'Receipt Student Updated successfully',
                'alert-type'=> 'success',
            );
            return redirect()->route('receipt_students.index')->with($notification);
    ...
    public function destroy($request)
    {
        try {
            ...
            $notification = array(
                'message' => 'Receipt Student Deleted successfully',
                'alert-type'=> 'error',
            );
            return redirect()->route('receipt_students.index')->with($notification);
        }
