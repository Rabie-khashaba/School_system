...
class ProcessingFeeRepository implements ProcessingFeeRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            DB::commit();
            $notification = array(
                'message' => 'Processing Fees Saved successfully',
                'alert-type'=> 'success',
            );
            return redirect()->route('ProcessingFee.index')->with($notification);
    ...
    public function update($request)
    {
        ...
        try {
            ...
            $students_accounts = StudentAccounts::where('processing_id',$request->id)->first();;
            $students_accounts->date = date('Y-m-d');
            $students_accounts->type = 'ProcessingFee';
            $students_accounts->student_id = $request->student_id;
            $students_accounts->processing_id = $ProcessingFee->id;
